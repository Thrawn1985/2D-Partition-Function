...
double cs_determinant ( const cs *A ) {
    int m = A->m;
    int n = A->n;

    if (m != n) {
        printf("error in determinant: matrix is not square\n");
        return 0;
    }

    css * S = cs_sqr (A, 0, 0) ;		/* ordering and symbolic analysis */
    csn * N = cs_lu (A, S, 1) ;
    ...
    for (int j = 0; j < u->n ; j++) {
        if (up[j] < up[j + 1]) {
            int p = up[j + 1] - 1; // only diagonal elements
            //printf("%f\n", ux[p]);
            d *= ux[p];
            log_d += log(fabs(ux[p]));
        }
